...
export const updateAnnounceSchema = z.object({
  ...
  breed: z
    ...
    .refine(val => !/\d/.test(val), {
      message: 'Порода не повинна містити цифри',
    })
    .optional(),
  animalName: z
    .string()
    ...
    .trim()
    .refine(val => !/\d/.test(val), {
      message: 'Ім’я не повинно містити цифри',
    })
    .optional(),
  animalLocation: z
    .string()
  ...
  adText: z
    .string()
    .min(10, 'Текст оголошення повинен мати мінімум 50 символів')
  ...
  images: z
    .custom<
      File[]
    >(files => !files || files.length === 0 || (files[0]?.size <= 5 * 1024 * 1024 && ['image/png', 'image/jpeg'].includes(files[0]?.type)))
    .optional(),
});
