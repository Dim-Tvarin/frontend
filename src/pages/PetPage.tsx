    ...
    <div className="flex gap-20 text-default-btn mt-100">
      ...
      <div className="w-1/2 flex flex-col  text-left">
        ...
        <div className="grid grid-cols-2 gap-x-auto gap-y-16 text-xl mb-32">
          ...
          <p className="text-base text-error">{animal?.status}</p>
          ...
          <div className="flex gap-5">
            ...
            {!!animal?.age.months && (
              <p className="text-base">{` ${animal?.age.months} міс.`}</p>
            )}
          </div>
          <p className="font-bold">Порода:</p>
          <p className="text-base">{animal?.breed}</p>
          <p className="font-bold">Де:</p>
          <p className="text-base">{animal?.animalLocation}</p>
          <p className="font-bold">Розмір:</p>
          <p className="text-base text-error">добавить в базу данные</p>
        </div>
        <p className="font-bold text-xl mb-16">Опис:</p>
        <p className="text-medium mb-32">{animal?.adText}</p>
        <div className="grid grid-cols-2 gap-y-16 text-xl mb-50">
          ...
          <p className="text-medium">{animal?.status}</p>
        </div>
        <CustomButton
